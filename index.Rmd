...
Click to [Download the Report](https://github.com/cchen941/PM566-finalproject/blob/main/FinalReport.pdf)
...
### Exploration of the relationship between death cases and sex&age.


#### Histogrom of Death Cases under 250
```{r echo=FALSE, warning=FALSE}
...
p4<-data4 %>% 
ggplot(aes(x=`COVID-19 Deaths`,y=`Age Group`,fill=Sex))+ geom_col(position= "dodge")
ggplotly(p4)
```

The second figure is the bar chart of death cases by age group, also colored by sex. It can be seen from this figure that the number of death cases shows an apparent increasing trend with the increasing age. In most age groups (except those over 85 years old), the number of deaths of male is more than that of female.
...
## Some Discussion
  ...
  * Try more interactive plots like map,scatter,heat map. Perform interactive tables. 
